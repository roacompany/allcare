...
- **Version**: **v2.8.10 (빌드 106) — App Store 출시 완료·라이브(`READY_FOR_SALE`)**. main `b35aecc`(PR #77) · **시리로 모든 기록**(기록 12종 전부 · 수면은 「잠들었어」/「깼어」로 앱 타이머 공유) + **#76 기록 출처 첫 출시**. PO 기기 QA 통과 후 2026-08-23 제출 → **2026-08-23 21:51Z 승인·자동 출시**(손님 스토어 iTunes lookup 실측 2026-08-24: 버전 2.8.10). 직전 라이브 **v2.8.9(빌드 105·2026-08-22 출시)를 교체**했다. v2.8.9 = **시리로 수유 기록(App Intents, #75)** 단일 기능 릴리스 — iOS 16+ App Shortcuts 라 손님이 단축어를 만들 필요가 없다(레퍼런스 베이비타임은 구형 Siri Shortcuts 4단계). **ASC 최고 빌드 = 106 VALID → 다음 main TF ≥107**(2026-08-23 실측). 임신 v3 는 컴파일 flag-off 휴면(노출 0). ⚠️ **2.8.10 train 도 닫혔다**(출시 완료) — 다음 fix 는 **v2.8.11** bump 필수 (build-gotchas.md `code 90186/90062`)
- **App Store**:
...
- **테스트**: 단위 테스트 함수 **736** (2026-08-23 실측 — `grep -rh "func test"`). `make verify` ALL CHECKS PASSED + 전 클래스 실행 **736개 통과·실패 0**(2 skip=사전 부채) + design 29/29 — 2026-08-23 `feat/siri-all-records` 실행. ⚠️ `make test` 는 `-quiet` 라 개수가 안 보인다 — 숫자를 보려면 `-only-testing:BabyCareTests` 를 `-quiet` 없이 따로 돌린다
- **규모**: **385** Swift 파일(앱 타깃) + 15(위젯), 23개 VM, 35개 Firestore 컬렉션 상수 (2026-08-23 실측)
