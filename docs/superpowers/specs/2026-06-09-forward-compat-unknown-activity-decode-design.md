...
3. `PatternAnalysis+Summary.swift:7` (`analyzeSummary`) — counts **all** activities → inflates the "종합 요약 / 총 기록 N건" card and adds an uncolored `.unknown` donut slice. **This also fixes a pre-existing `.pumping` leak** (pumping is wrongly counted here today), so the positive whitelist is the correct fix, not a `!= .unknown` patch.
