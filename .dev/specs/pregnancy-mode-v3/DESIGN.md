...
- **🔴 열린 결정 — 파트너 쓰기 범위**:
  - (A) **비대칭**(zero-base 권고): 의료수치(체중/검진/증상)=산모 owner-write, 파트너=읽기+감정/코멘트(태담·스탬프). 충돌 표면 0.
  - (B) **대칭**(PO 1차 선택): 둘 다 모든 기록. 단일트리+createdBy라 유실 없음. 단 동일 레코드 동시편집 LWW 충돌 표면 추가.
  - → **구현 전 확정 필요.**
...
## 10. 구현 전 PO 결정
1. **파트너 쓰기 범위** — 비대칭(권고) vs 대칭 (§2.1)
2. **③한국 검진/바우처/산모수첩 데이터 큐레이션** — 최대 신규작업, 외부 데이터
3. **RC 단계 롤아웃** — 5→25→100% (v2.7.1 5빌드 회귀 방지)
4. **v2 `.both` 산모 마이그레이션 코치마크** 1회
+ (전략) zero-base가 짚은 진짜 관문 = **의사·변호사 감수**(H-items, PO "섭외 가능" 확인) — 코드 아닌 이 게이트가 v1·v2를 죽였음.
