...
- **뿌리 A**: 하루 조회는 「그날 시작한 것」+「그날 끝난 것」 두 쿼리를 합쳐 온다(달력 점을 양쪽에 찍으려는 의도된 설계). 세는 쪽이 그걸 모르면 같은 기록이 두 날에서 통째로 세어진다
- **뿌리 B**: 반대로 기간 조회는 시작 시각만 묻는다 — 기간 첫날 새벽에 걸친 잠이 아예 안 온다. 달력만 하루 앞당겨 부르고 있었다
- **시간은 자정에서 잘라 두 날이 나눠 갖고**(`ActivityDayAttribution.totalClippedDuration`), **횟수·양은 나눌 수 없으니 시작한 날 하루에만**(`startedOn` / `startedWithin`). 「그날 총 몇 시간」을 답하던 자리가 넷인데 셋만 클립을 쓰고 있었다 — 전부 한 판정으로
- 고친 화면: 달력 일일 요약(수면 시간·수유 횟수/양) · 홈 오늘 요약 + **위젯** · **유축 모유 재고**(같은 수유가 두 번 차감돼 80ml 가 0ml 로) · 패턴 리포트 · 병원 리포트 PDF · 통계 · 낮잠/밤잠 비율
- 자정 직후 「마지막 …」: 수유에만 있던 fallback 을 수면·기저귀에도 — 1시간 전에 간 기저귀가 "오늘 기록 없음"이었다
- 기간 조회를 하루 앞당기면서 그 하루가 **유령 막대**로 새지 않도록 「기간 안에서 시작한 것」(횟수·막대)과 「기간에 걸친 것」(시간 합계)을 분리
- Fixed: 「지난주 대비」가 사과 대 오렌지였다 — 이번 주는 클립해 재는데 지난주는 통째로 더했다
- Fixed(예방): 로그아웃 때 최근 7일 기록이 안 지워지고 있었다(계정 전환 시 남의 기록이 「마지막 기저귀」로). `ActivityViewModel.reset()` 로 모으고 **지울 목록을 속성 옆**에 뒀다 — 딴 파일에 나열하면 다음 캐시가 조용히 빠진다
- Fixed(예방): `recentFeedingActivities` 를 파생시켜, 두 목록이 갈라져 한쪽만 채운 채 물으면 빈칸이 되던 구조를 제거
- 게이트: **745 tests / 실패 0**(736 → +9) · arch R1-4=0 · lint 0 err. 전부 RED 재현 후 수정. 달력·통계·패턴 리포트에 같은 밤잠을 물어 **셋이 같은 값을 내는지 대조**하는 테스트로 재분기 봉쇄
- ⚠️ 데이터 마이그레이션 없음 — 저장된 startTime/endTime 은 그대로, **세는 방식만** 바뀐다

## [2.8.10] - 2026-08-23 (App Store 출시 — 빌드 106)
...
> **→ 2026-08-23 21:51Z 승인·자동 출시 완료(`READY_FOR_SALE`)** — 손님이 보는 스토어도 2.8.10 확인
> (iTunes lookup 실측 2026-08-24). 라이브 v2.8.9 를 교체했다.
> ⚠️ v2.8.9 출시로 train 이 닫혀 2.8.10 으로 올렸다. **이제 2.8.10 train 도 닫혔다 — 다음 fix 는 v2.8.11.**
...
- **수면 = 「잠들었어」/「깼어」**: 새 개념을 만들지 않고 앱의 기존 수면 타이머를 시리와 공유한다(`RunningTimerStore` — 타이머 상태를 App Group 으로 이관 + 옛 자리에서 1회 마이그레이션). 이미 자는 중이면 덮어쓰지 않고(앞의 잠이 사라진다), 다른 타이머가 돌면 그것을 끄지 않는다. 24시간을 넘기면 폼과 같은 문구로 거절하고 **타이머는 남긴다**(앱에서 시간을 고쳐 저장할 수 있게)
- **문구에 종류를 변수로**(`\(\.$kindOption)`): App Shortcut 은 앱당 10개 제한이라 12종을 하나씩 넣을 수 없다 → 자주 쓰는 5종 프리셋 + 수면 2 + **종류 변수 1** = 8자리로 전 종류를 덮는다
- **되묻기 판정을 하나로**(`SiriPromptPolicy`): 이 종류가 무엇을 물어야 하는지가 한 자리에만 있고, `ActivityDraftBuilder` 와 어긋나면 게이트가 빨강이 된다(전 종류 순회 대조 — 일부러 틀리게 해 빨강 확인)
- Fixed(예방): 로그아웃 정리를 **접두 규약**으로 바꿨다(`SiriSharedStore.clearAll()` — `siri_` 로 시작하는 App Group 키를 전부 지운다). 지울 것을 이름으로 나열하던 자리라 새 시리 값이 조용히 빠질 구조였다 — 이제 목록 자체가 없다
- Fixed: 시리 응답 조사(`KoreanParticle`) — 수유 낱말만 있을 땐 늘 '를' 이 맞아 안 보이던 결함. 전 종류로 넓히자 "소변를 기록했어요" 가 됐다. 받침에 따라 을/를·이/가를 고른다
- Removed: 죽은 판정 2종(`ActivityType.needsAmount` · `needsQuickInput`) — 프로덕션 호출 0, `RecordEntryRule` 이 이미 대체했다
- 종류 누락 방지: 기록 런처에 타일을 더하면 시리에 빠졌을 때 게이트가 빨강이 된다(이름 열거 대신 대조)
- 옛 인텐트(`RecordFeedingIntent`)는 **한 판 남긴다** — 지우면 손님이 단축어 앱에 만들어 둔 것이 깨진다. 판정은 새 것과 같은 자리를 쓴다(문은 둘, 판정은 하나)
- **데이터 모델 변경 0 · 마이그레이션 0** — 저장 규칙·통계·유축 재고·병원 리포트 계약 그대로
- ⚠️ 아기가 여럿이면 **앱에서 고른 아기**에만 기록된다(현행 제약 유지) · 성장·일기·접종은 범위 밖
- 게이트: `make verify` ALL CHECKS PASSED · **736 tests / 실패 0**(704 → +32, 2 skip=사전 부채) · arch R1–R4=0 · design 29/29
...
## [2.8.9] - 2026-08-22 (App Store 출시 — 빌드 105)
...
> versionId `41a6f84b-5615-4eea-98b6-7b8615800e65`). **→ 2026-08-22 승인·자동 출시 완료(`READY_FOR_SALE`)**,
> 2026-08-23 v2.8.10 으로 교체됨. 원래는 시뮬레이터에서 시리가 제대로 동작하지 않아 코드·CI 로는 확인할 수 없는 구간(**앱 미실행 상태의 백그라운드 실행 · 인증 복원**)을 실기기에서 재기 위함. 2.8.8 train 이 닫혀 2.8.9 로 올렸다. App Store 제출은 PO 결정.
...
- **라이브 폼 원복(PO 결정)**: 빌드 100~103의 재설계 런처(`RecordLauncherSheet`) 제거, 기록 화면을 라이브 v2.8.6 폼 기준으로 복원(＋탭·캘린더 진입 통일). 저장 파이프라인(`ActivityDraft`/`commit`)·유축 재고는 유지
- **용어 확정('짜기' 금지)**: 짜는 행위=**유축**(생산) · 짜둔 모유 먹이기=**모유(병)**(섭취) — 세그먼트 3곳(빠른기록·풀폼·편집시트) `FeedingContent.displayName` 단일 소스 통일 · 홈 타임라인 `displayLabel` 적용(모유(병)이 '분유'로 표기되던 이원화 수리) · 물질명 '짜둔 모유'→'유축한 모유' 7곳
- **한 활동=한 색**: 강조색 자산 9종(라이트/다크) + `Activity.emphasisColor` 단일 소스 — 폼 3곳의 색 하드코딩 제거(분유=인디고·간식=세이지 이원화 해소) · 미선택 탭·칩=중립+타입색 아이콘
- **기저귀 정본화**: 소변=앰버 물방울 · 대변=브라운 습기(잎사귀 아이콘 제거) · 소변+대변=앰버 배지(폼 카드+홈 타일)
- **폼 구조**: 3중 제목(typeHeader) 제거 · 저장 버튼 하단 고정(`SaveBar`, 전 폼) · 유축 안내 압축(선택 보관의 유통기한 한 줄 + '왜 유축은 먹인 기록이 아닌가요?' 접기 — 초안·참고용 면책 유지)
- **유축 허브(A안)**: 유축 시트 [유축|먹이기] 모드 — 먹이기는 모유(병)으로 저장(섭취 통계·병원 리포트·재고 FIFO 차감 계약 그대로, 데이터 모델 무변경)
- **이어주기(B안)**: 유축 저장 직후 토스트 '모유(병)?' 제안(`NextRecordSuggestionPolicy` 타일 반환) · 재고 화면 하단 '먹이기 기록' 버튼
- Fixed: `neutralGrayColor` 자산 Any 항목 누락(잠재 결함) 수리 + 강조색 자산 실존 단위 게이트 추가

### Added/Changed — 기록하기 통합 재설계 + 유축 재고 (빌드 100~103, 2026-07-12)

- **기록하기 전면 재설계**: 저장 경로 3벌→1벌(`ActivityDraft`/`commit`) · 타입-우선 ＋런처(`RecordLauncherSheet`) + 단일 상세 시트(`UnifiedRecordSheet`) · 연속 기록 + 되돌리기 토스트 · 죽은 폼 9파일 제거 (빌드 100~102)
- **수유 용어 정리**: 모유수유→모유 · 유축(생산)→짜기 · 병수유 모유→유축 · 같은 병수유를 분유/유축 두 타일로 분리(`RecordTile` content 프리셋). 수량 명사 '유축량'은 병원 리포트 관용어라 유지
- **유축 재고**: 짜기 시 보관(실온/냉장/냉동) 선택 · 대시보드 재고 카드(총량·임박 만료) · 재고 화면(배치 목록·신선도 뱃지·폐기 스와이프) · 유축 먹이기 시 FIFO 잔량 차감 · 유통기한 임박 로컬 알림(설정 토글, 기본 ON)
- ⚠️ 유통기한 숫자(실온 4h·냉장 4일·냉동 6개월)는 의료 감수 전 초안 + 참고용 면책 동반
